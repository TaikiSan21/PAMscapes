...
```{r}
# Can load MANTA NetCDF using this
manta <- checkSoundscapeInput(here('tutorial/MANTAExample.nc'))
ol <- checkSoundscapeInput(here('tutorial/ADRIFT_017_OL_sim.csv'))
cat(str(manta, list.len=10))
str(ol)
```
...
Here we'll show how this works for the small set of MANTA data we just loaded, which outputs in HMD
resolution.
...
density distribution (`style='density'`). The MANTA data has a "0" frequency, and we can't show 0 on a
log scaled frequency axis (the default), so we're going to get some warnings here - that's okay.

```{r}
(plotPSD(manta, style='quantile', q=.05)) /
...
```{r}
combinedOl <- prepPSDData(list(ol, mantaOl))
```

We get a warning that they cannot be combined - even though the octave level measurements are present
at the same intervals, the `mantaOl` measurements stop at 4kHz. We can get around this by limiting `ol`
to the columns up to 4kHz.

```{r}
combinedOl <- prepPSDData(list(ol[1:6], mantaOl))
(plotPSD(ol, style='density', dbRange=c(50, 130)) +
    plotPSD(mantaOl, style='density', dbRange=c(50, 130))) /
...
```{r}
(plotPSD(manta, style='quantile', q=0, freqRange=c(100,1e4)) + 
     plotPSD(manta, style='quantile', q=c(.01, .8), color='red', freqRange=c(100, 1e4))) /
    (plotPSD(manta, style='density', cmap=scales::viridis_pal(option='B')(25), dbRange=c(30, 120)) +
         plotPSD(manta, style='density', cmap=scales::viridis_pal(option='mako')(25), freqRange=c(1e2, 1e3)))
```
...
```{r}
plotPSD(manta, style='density', units='TOL dB re 1uPa', title='Linear Plot', scale='linear')
```

You can also plot the quantile lines overlaid on top of the density heatmap.

```{r}
plotPSD(manta, style=c('quantile', 'density'), q=.05, color='red')
```
...
Here we'll some TOL data from MANTA on the same plot as some TOL metrics from Triton.
...
```{r}
tolManta <- createOctaveLevel(manta, type='tol', freqRange=c(125, 4e3))
# we'll use the "Source" column for our labeling
tolManta$Source <- 'MANTA'
tol <- checkSoundscapeInput(here('tutorial/ADRIFT_017_TOL_sim.csv'))
tol$Source <- 'Triton'
# columns 18-24 contain frequency higher than the 4kHz upper limit of our
# MANTA data, so we'll remove them before combining
combined <- rbind(tolManta, tol[-(18:24)])
